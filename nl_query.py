...
def _to_days(multiplier: int, group_index: int) -> _TimeConverter:
    def convert(match: re.Match[str]) -> int:
        return int(match.group(group_index)) * multiplier

    return convert


_TIME_PATTERNS: tuple[_PatternSpec, ...] = (
    (re.compile(r"(최근|지난)\s*(\d+)\s*일"), _to_days(1, 2)),
    (re.compile(r"(최근|지난)\s*(\d+)\s*주"), _to_days(7, 2)),
...
def _extract_limit(text: str) -> tuple[int | None, str]:
    ...
    return limit, _remove_span(text, best_match.start(), best_match.end())


def parse_query(raw: str) -> ParsedQuery:
    text = raw.strip()
    days, without_time = _extract_time(text)
    limit, without_filters = _extract_limit(without_time)
    search_text = re.sub(r"\s+", " ", without_filters).strip()
